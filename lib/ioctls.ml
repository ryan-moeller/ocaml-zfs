...
external pool_create :
  handle -> string -> bytes -> bytes option -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_pool_create"

(* pool_destroy handle name log_msg *)
external pool_destroy :
  handle -> string -> string -> (unit, Unix.error) Result.t
...
external pool_export :
  ...
  bool ->
  string option ->
  (unit, Unix.error) Result.t = "caml_zfs_ioc_pool_export"

(* pool_configs handle ns_gen *)
external pool_configs :
  handle -> int64 -> ((int64 * bytes) option, Unix.error) Result.t
  = "caml_zfs_ioc_pool_configs"

(* pool_stats handle name *)
external pool_stats :
  handle -> string -> (bytes, bytes option * Unix.error) Result.t
  = "caml_zfs_ioc_pool_stats"

(* pool_tryimport handle packed_config *)
external pool_tryimport : handle -> bytes -> (bytes, Unix.error) Result.t
...
external pool_scan :
  ...
  pool_scan_func ->
  pool_scrub_cmd ->
  (unit, Unix.error) Result.t = "caml_zfs_ioc_pool_scan"

(* pool_freeze handle name *)
external pool_freeze : handle -> string -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_pool_freeze"

(* pool_upgrade handle name version *)
external pool_upgrade : handle -> string -> int64 -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_pool_upgrade"

(* pool_get_history handle name offset *)
external pool_get_history :
  handle -> string -> int64 -> (bytes option, Unix.error) Result.t
  = "caml_zfs_ioc_pool_get_history"

(* vdev_add handle name packed_config check_ashift *)
external vdev_add :
  handle -> string -> bytes -> bool -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_vdev_add"

(* vdev_remove handle name guid *)
external vdev_remove : handle -> string -> int64 -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_vdev_remove"

(* vdev_remove_cancel handle name *)
external vdev_remove_cancel : handle -> string -> (unit, Unix.error) Result.t
...
external vdev_set_state :
  ...
  vdev_state ->
  int64 ->
  (vdev_state, Unix.error) Result.t = "caml_zfs_ioc_vdev_set_state"
...
external objset_stats :
  handle -> string -> bool -> (objset_stats * bytes option, Unix.error) Result.t
  = "caml_zfs_ioc_objset_stats"

(* objset_zplprops handle name *)
external objset_zplprops : handle -> string -> (bytes, Unix.error) Result.t
...
external dataset_list_next :
  ...
  bool ->
  int64 ->
  ((string * objset_stats * bytes option * int64) option, Unix.error) Result.t
...
external set_prop :
  handle -> string -> bytes -> (unit, bytes option * Unix.error) Result.t
  = "caml_zfs_ioc_set_prop"

(* create handle name packed_args *)
external create : handle -> string -> bytes -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_create"

(* destroy handle name defer *)
external destroy : handle -> string -> bool -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_destroy"

(* rollback handle name packed_args *)
external rollback :
  handle -> string -> bytes option -> (bytes, Unix.error) Result.t
...
external rename :
  ...
  string ->
  rename_flag array ->
  (unit, string option * Unix.error) Result.t = "caml_zfs_ioc_rename"
...
external recv :
  ...
  bytes ->
  bool ->
  (int64 * zprop_errflag array * bytes, Unix.error) Result.t
...
external inject_list_next :
  handle ->
  int64 ->
  ((int64 * string * zinject_record) option, Unix.error) Result.t
  = "caml_zfs_ioc_inject_list_next"

(* error_log handle string *)
external error_log :
  handle -> string -> (zbookmark_phys array, Unix.error) Result.t
...
external clear :
  ...
  int64 option ->
  bytes option ->
  (bytes option, Unix.error) Result.t = "caml_zfs_ioc_clear"

(* promote handle name *)
external promote :
  handle -> string -> (unit, string option * Unix.error) Result.t
...
external dsobj_to_dsname :
  handle -> string -> int64 -> (string, Unix.error) Result.t
...
external pool_set_props :
  handle -> string -> bytes -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_pool_set_props"

(* pool_get_props handle name *)
external pool_get_props : handle -> string -> (bytes, Unix.error) Result.t
  = "caml_zfs_ioc_pool_get_props"

(* set_fsacl handle name un packed_acl *)
external set_fsacl :
  handle -> string -> bool -> bytes -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_set_fsacl"

(* get_fsacl handle name *)
external get_fsacl : handle -> string -> (bytes, Unix.error) Result.t
...
external inherit_prop :
  handle -> string -> string -> bool -> (unit, Unix.error) Result.t
...
external userspace_many :
  ...
  int ->
  int64 ->
  (int64 * useracct array, Unix.error) Result.t = "caml_zfs_ioc_userspace_many"

(* userspace_upgrade handle name *)
external userspace_upgrade : handle -> string -> (unit, Unix.error) Result.t
...
external get_holds : handle -> string -> (bytes, Unix.error) Result.t
  = "caml_zfs_ioc_get_holds"

(* objset_recvd_props handle name *)
external objset_recvd_props : handle -> string -> (bytes, Unix.error) Result.t
...
external next_obj :
  handle -> string -> int64 -> (int64 option, Unix.error) Result.t
  = "caml_zfs_ioc_next_obj"

(* diff handle to from fd *)
external diff :
  handle -> string -> string -> Unix.file_descr -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_diff"

(* tmp_snapshot handle name prefix cleanup_fd *)
external tmp_snapshot :
  handle -> string -> string -> Unix.file_descr -> (string, Unix.error) Result.t
  = "caml_zfs_ioc_tmp_snapshot"

(* obj_to_stats handle name obj *)
external obj_to_stats :
  handle -> string -> int64 -> (string * stat, Unix.error) Result.t
  = "caml_zfs_ioc_obj_to_stats"

(* space_written handle name snap *)
external space_written :
  handle -> string -> string -> (int64 * int64 * int64, Unix.error) Result.t
  = "caml_zfs_ioc_space_written"

(* space_snaps handle lastsnap packed_args *)
external space_snaps : handle -> string -> bytes -> (bytes, Unix.error) Result.t
...
external pool_reguid : handle -> string -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_pool_reguid"

(* pool_reopen handle name packed_args *)
external pool_reopen :
  handle -> string -> bytes option -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_pool_reopen"

(* send_progress handle name fd *)
external send_progress :
  handle -> string -> Unix.file_descr -> (int64 * int64, Unix.error) Result.t
  = "caml_zfs_ioc_send_progress"

(* log_history handle packed_args *)
external log_history : handle -> bytes -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_log_history"

(* send_new handle tosnap packed_args *)
external send_new : handle -> string -> bytes -> (unit, Unix.error) Result.t
...
external recv_new : handle -> string -> bytes -> (bytes, Unix.error) Result.t
  = "caml_zfs_ioc_recv_new"

(* pool_sync handle name packed_args *)
external pool_sync : handle -> string -> bytes -> (unit, Unix.error) Result.t
...
external channel_program :
  ...
  bytes ->
  int64 ->
  (bytes, bytes option * Unix.error) Result.t = "caml_zfs_ioc_channel_program"

(* load_key handle name packed_args *)
external load_key : handle -> string -> bytes -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_load_key"

(* unload_key handle name *)
external unload_key : handle -> string -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_unload_key"

(* change_key handle name packed_args *)
external change_key : handle -> string -> bytes -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_change_key"

(* remap not implemented on openzfs *)

(* pool_checkpoint handle name *)
external pool_checkpoint : handle -> string -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_pool_checkpoint"

(* pool_discard_checkpoint handle name *)
external pool_discard_checkpoint :
  handle -> string -> (unit, Unix.error) Result.t
...
external redact : handle -> string -> bytes -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_redact"

(* get_bookmark_props handle name *)
external get_bookmark_props : handle -> string -> (bytes, Unix.error) Result.t
  = "caml_zfs_ioc_get_bookmark_props"

(* wait handle name packed_args *)
external wait : handle -> string -> bytes -> (bytes, Unix.error) Result.t
  = "caml_zfs_ioc_wait"

(* wait_fs handle name packed_args *)
external wait_fs : handle -> string -> bytes -> (bytes, Unix.error) Result.t
  = "caml_zfs_ioc_wait_fs"

(* vdev_get_props handle name packed_args *)
external vdev_get_props :
  handle -> string -> bytes -> (bytes, Unix.error) Result.t
...
external pool_scrub : handle -> string -> bytes -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_pool_scrub"

(* nextboot handle packed_args *)
external nextboot : handle -> bytes -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_nextboot"

(* jail handle name jid *)
external jail : handle -> string -> int -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_jail"

(* unjail handle name jid *)
external unjail : handle -> string -> int -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_unjail"

(* set_bootenv handle name packed_args *)
external set_bootenv : handle -> string -> bytes -> (unit, Unix.error) Result.t
  = "caml_zfs_ioc_set_bootenv"

(* get_bootenv handle name *)
external get_bootenv : handle -> string -> (bytes, Unix.error) Result.t
